...
def send_message():
    ...
    # get message from message entry
    ...
    if message:
        try:
            ...
            # insert message into chat area
            chat_area.config(state=tk.NORMAL)
            chat_area.insert(tk.END, f"You ({name}): {message}\n")
            chat_area.config(state=tk.DISABLED)
            chat_area.yview(tk.END)
        ...
        # close connection if message is [bye]
        if message == "[bye]":
            sleep(0.1)              # wait for message to send
            client_socket.close()
            chat_window.quit()
